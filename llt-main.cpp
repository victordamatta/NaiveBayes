...
int main (int argc, char *argv[]) {
    ...
    map<string, string> llt_to_pt;

    ifstream f0 ("dados/llt_to_pt.asc");
    while (getline (f0, line)) {
        size_t sep = line.find ("$");
        string llt = line.substr (0, sep);
        string pt = line.substr (sep + 1, line.size() - sep - 2);
        llt_to_pt[llt] = pt;
    }

    ifstream f1 ("dados/llt_without_marks.asc");
    ...
    */
    ...
    while (getline (cin, line)) {
        ...
        string guess = llt_to_pt[classify (split (verb), dics)];
